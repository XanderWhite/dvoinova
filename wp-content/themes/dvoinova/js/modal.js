...
const areementCheckboxes = document.querySelectorAll(
	".modal__form__agreement__checkbox"
);
...
// Удаляем класс error у span имитирующего checkbox
areementCheckboxes.forEach((checkbox) => {
	checkbox.addEventListener("change", function (e) {
		const span = e.target
			.closest(".modal__form__agreement")
			.querySelector(".modal__form__agreement__span");
    ...
    inputs.forEach(input => {
        ...
        if (input.classList.contains("email") && isEmailInvalid(input)) {
            ...
            const errorSpan = input
				...
				.querySelector(".modal__form__agreement__span");
			if (errorSpan) {
				formAddError(errorSpan); // Передаем span вместо input
				error++;
			}
